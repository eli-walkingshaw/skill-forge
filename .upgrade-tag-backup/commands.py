...
def wrap_for_obsidian(skill_md: str, header_lines: list[str]) -> str:
    ...
    callout = ["> [!info] skill-forge proposal"] + [f"> **date:** {date_str}"] + [f"> {l}" for l in header_lines] + [
        ">",
...
def proposal_filename(config: Config, name: str, gist: str = "") -> Path:
    ...
    The date lives inside the file's callout header, not in the filename,
    so Obsidian's sidebar stays readable.
    Collisions on the same name get -2, -3, etc.
    """
...
_FILLER_WORDS = {
    ...
    "fix", "fixing", "fixed",  # too generic on their own
}
...
def _shorten_for_filename(text: str, max_words: int = 4, max_chars: int = 32) -> str:
    ...
    if not text:
        return ""
    # Strip code-ish noise that adds no meaning.
    text = re.sub(r"[`\\]+", " ", text)
    words = re.findall(r"[A-Za-z0-9]+", text.lower())
    # Keep meaningful words, in original order, deduped.
    kept = []
    seen = set()
    for w in words:
        ...
        if len(kept) >= max_words:
            break
    # If we filtered too aggressively, fall back to the first words verbatim.
    if not kept:
...
def cmd_new(args, config: Config) -> int:
    ...
    out = proposal_filename(config, name, gist=args.name)
...
Write the SKILL.md."""
    ...
    out = proposal_filename(config, name, gist=description)
    out.write_text(wrapped, encoding="utf-8")
    print(f"✓ {out}")
...
Turn this into a SKILL.md, or respond NOT_A_SKILL if it isn't skill-worthy."""
    ...
    out = proposal_filename(config, name, gist=note_path.stem)
    out.write_text(wrapped, encoding="utf-8")
    print(f"✓ {out}")
    return 0
