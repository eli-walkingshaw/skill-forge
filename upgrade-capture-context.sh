#!/usr/bin/env bash
# Capture context-handling upgrade:
# - tells the model the context might be irrelevant (so it doesn't strain to use it)
# - model reports back which context items it actually used
# - the proposal callout shows BOTH what was grabbed and what was used
# - filename gist (--note) is now actually applied to capture proposals
set -euo pipefail

[[ -f forge/__main__.py ]] || { echo "Run from ~/code/skill-forge"; exit 1; }

mkdir -p .upgrade-context-backup
cp forge/commands.py .upgrade-context-backup/ 2>/dev/null || true
echo "  ✓ backed up forge/commands.py to .upgrade-context-backup/"
...
A) Is this skill-worthy? A SOLVED pattern that they'd benefit from Claude knowing next time, OR a workflow/convention/recipe worth capturing. The note must contain BOTH the problem AND the resolution (or enough to imply both). A note that's just a symptom/topic with no solution → NOT_A_SKILL.

B) If NOT skill-worthy, respond with EXACTLY ONE LINE:

NOT_A_SKILL: <one-line reason, friendly, names what's missing>

C) If skill-worthy, write a complete SKILL.md prefixed by a single header line that names which context items you actually used. The exact format is:

USED_CONTEXT: <comma-separated list, or "none">
---
name: short-kebab-case-name
description: ONE PARAGRAPH. What the skill does AND when it should trigger. Concrete keywords from the note (and relevant context only) — file names, error strings, tool names, user phrases. Slightly pushy.
---
...
## When to use
Bulleted triggers, drawn from the note. Be specific.

## The pattern
The reusable knowledge. Code blocks if relevant code appeared in the note or in genuinely-related context.

## Steps
Numbered actions.

## Gotchas
Surprises and past mistakes inferable from what the user wrote.

Context label vocabulary for USED_CONTEXT (use only the labels that appeared and were actually relevant):
- clipboard
- shell-history
...
- If the user's note is too thin AND the context doesn't fill the gap, say NOT_A_SKILL.
- NEVER invent details to fill gaps. Better to NOT_A_SKILL than to draft something speculative.
- If existing skills are provided as style examples, MATCH their tone and specificity.
- The USED_CONTEXT line must come first, then a blank line, then the SKILL.md frontmatter. No preamble before USED_CONTEXT."""
...
Decide: write the SKILL.md, or respond NOT_A_SKILL."""
    ...
    if raw.startswith("NOT_A_SKILL"):
        print()
        print(raw)
        # Save the note to inbox so it's not lost.
        inbox_save = config.inbox_dir / f"capture_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.md"
        inbox_save.parent.mkdir(parents=True, exist_ok=True)
        inbox_save.write_text(
            f"---\ngoal: {note[:80]}\n---\n\n{note}\n\n## Context\n\n{ctx.to_prompt_block()}\n",
            encoding="utf-8",
        )
        print(f"\n(note saved to {inbox_save.name} for future reference)")
        return 0

    skill_md = strip_outer_fence(raw)

    # The model is asked to prefix its output with "USED_CONTEXT: <list>" on
    # its own line, followed by the SKILL.md. Pull it apart.
    used_context = _parse_used_context(skill_md)
    skill_md = _strip_used_context_header(skill_md)
...
def _parse_used_context(text: str) -> str:
    ...
    m = _USED_CONTEXT_RE.search(text[:300])  # only check the head
    if not m:
        return ""
    raw = m.group(1).strip()
...
def _strip_used_context_header(text: str) -> str:
    """Remove the leading USED_CONTEXT line so what we save is pure SKILL.md.

    The model is asked to put USED_CONTEXT on its own line at the very top,
    optionally followed by a blank line. Frontmatter starts immediately after.
    We strip both the USED_CONTEXT line and any trailing blank lines before
    the frontmatter.
    """
    ...
    n = len(lines)
    saw_header = False
    # Skip leading blank lines before USED_CONTEXT (defensive)
    while i < n and not lines[i].strip():
        i += 1
    # Strip the USED_CONTEXT line itself if present
    if i < n and _USED_CONTEXT_RE.match(lines[i]):
        saw_header = True
        i += 1
        # Strip blank lines that may follow it
        while i < n and not lines[i].strip():
            i += 1
    if not saw_header:
        # No header found — return original unchanged.
        return text
    result = "\n".join(lines[i:])
...
echo "✓ done"
echo ""
echo "Try a real capture now:"
echo "  python3 -m forge capture --note \"Suitelet rendered white screen because the inline SVG data URI had a literal # character. Fix: percent-encode to %23.\""
