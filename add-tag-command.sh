#!/usr/bin/env bash
# Adds `forge tag` — auto-tags all installed skills with a shared vocabulary
# so Obsidian connects related ones via its tag pane.
set -euo pipefail

[[ -f forge/__main__.py ]] || { echo "Run from ~/code/skill-forge"; exit 1; }

mkdir -p .upgrade-tag-backup
cp forge/commands.py forge/__main__.py .upgrade-tag-backup/ 2>/dev/null || true
echo "  ✓ backed up to .upgrade-tag-backup/"
...
echo ""
echo "Verifying..."
python3 -m forge --help | grep -E "tag|capture|digest" | head -5

echo ""
echo "✓ done"
echo ""
echo "Try:"
echo "  python3 -m forge tag             # dry-run — see what tags would be assigned"
echo "  python3 -m forge tag --apply     # actually write + commit + push"
echo ""
echo "Dry-run first. If the tags look reasonable, then --apply."
