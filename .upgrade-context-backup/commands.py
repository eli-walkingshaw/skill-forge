...
def list_installed_skills(config: Config) -> list[Path]:
    """Return paths to the skill file inside each skill folder.

    Accepts either:
      - <repo>/<name>/SKILL.md   (Anthropic convention)
      - <repo>/<name>/<name>.md  (alternate convention some setups use)

    If both exist in the same folder, SKILL.md wins.
    """
    ...
    for child in sorted(repo.iterdir()):
        if not child.is_dir() or child.name.startswith("."):
            continue
        # Prefer SKILL.md if present.
        skill_md = child / "SKILL.md"
        if skill_md.exists():
            found.append(skill_md)
            continue
        # Fall back to <name>/<name>.md
        same_name = child / f"{child.name}.md"
        if same_name.exists():
            found.append(same_name)
    return found
