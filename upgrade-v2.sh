#!/usr/bin/env bash
# Upgrade skill-forge to v2 (capture + digest + install-hotkey).
...
mkdir -p .upgrade-v2-backup
cp forge/__main__.py forge/commands.py .upgrade-v2-backup/ 2>/dev/null || true
echo "  ✓ saved to .upgrade-v2-backup/"

echo ""
echo "Writing forge/context.py (new)..."
cat > forge/context.py << 'CONTEXT_EOF'
...
echo "Writing forge/commands.py (updated)..."
...
CAPTURE_SYSTEM = """You are a skill author for Claude. The user just solved something and wrote a short note about it, along with auto-grabbed context from their environment (clipboard, shell history, last Claude exchange).

Decide:

A) Is this skill-worthy? A SOLVED pattern that they'd benefit from Claude knowing next time, OR a workflow/convention/recipe worth capturing.

B) If NOT skill-worthy (just a question, half-formed thought, or one-off), respond with:

NOT_A_SKILL: <one-line reason, friendly>

C) If skill-worthy, write a complete SKILL.md. Use the note as the primary signal — the context is supporting evidence to flesh out specifics. The user's note tells you WHAT was solved; the context tells you HOW.

SKILL.md structure:

---
name: short-kebab-case-name
description: ONE PARAGRAPH. What the skill does AND when it should trigger. Concrete keywords from the note and context — file names, error strings, tool names, user phrases. Slightly pushy.
---
...
## When to use
Bulleted triggers (drawn from the user's note + context — be specific to their domain).

## The pattern
The reusable knowledge. Code blocks if relevant code appeared in context.

## Steps
Numbered actions.

## Gotchas
Surprises and past mistakes (often inferable from what the user wrote).

Rules:
- Keep under 200 lines.
- If the user wrote almost nothing, you may need to lean on context — but don't invent. If the signal isn't strong enough, say NOT_A_SKILL.
- If existing skills are provided as style examples, MATCH their tone and specificity.
- Output ONLY the SKILL.md content (or the NOT_A_SKILL line). No preamble."""
...
echo "Writing forge/__main__.py (updated)..."
...
echo ""
echo "Verifying CLI loads..."
python3 -m forge --help | head -20

echo ""
echo "✓ upgrade v2 complete"
echo ""
echo "Three new commands available:"
echo ""
echo "  forge capture              # opens dialog, auto-grabs context, drafts SKILL.md"
echo "  forge capture --note \"...\"  # same but with note as arg"
echo "  forge digest               # weekly digest of 'resolution-shaped' moments from Claude Code logs"
echo "  forge install-hotkey       # prints macOS Quick Action setup instructions"
echo ""
echo "Try first: forge capture --note \"test capture from upgrade\""
